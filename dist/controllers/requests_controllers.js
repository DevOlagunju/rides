...
var createRequest = function createRequest(req, res) {
  ...
  client.query("SELECT rideId,user_id,available_seats FROM rides WHERE rideId=".concat(rideId), function (err, resp) {
    var seats;
    ...
    if (resp.rows.length) {
      if (resp.rows[0].available_seats === 0) {
        ...
        client.query("INSERT INTO requests ( ride_Id,usersId,passenger_name,phone_no,) VALUES (\n                 '".concat(rideId, "','").concat(userId, "','").concat(passenger_name, "','").concat(phone_no, "')"), function (error, response) {
          if (error) {
            res.status(409).jsend.fail({
              code: 409,
          ...
          if (response) {
            client.query("UPDATE rides SET available_seats=".concat(seats, " WHERE rideId=").concat(rideId), function (anyError) {
              if (anyError) {
...
var getAllRequest = function getAllRequest(req, res) {
  ...
  if (!errors.isEmpty()) {
    ...
    client.query("SELECT * FROM requests where ride_id = ".concat(rideId), function (err, requests) {
      if (err) {
...
var acceptOrRejectRequest = function acceptOrRejectRequest(req, res) {
  ...
  client.query("UPDATE requests SET status = $1 WHERE requestId = ".concat(requestId, " AND ride_id = ").concat(rideId), [status], function (err, request) {
    if (err) {
      res.send(err);
      console.log(err);
    } else {
