...
document.querySelector("#nameBar").innerHTML = "".concat(firstname.toUpperCase()); //fetch request to render all user rides into the table

var userId = localStorage.getItem("userId");
fetch("/users/".concat(userId, "/rides"), {
  method: "GET",
  headers: {
    Authorization: token
  }
  ...
  if (!data.length) {
    document.querySelector("#error-msg").innerHTML = "You do not have any Ride Offer yet";
  } else {
    data.sort(function (a, b) {
      return a.id - b.id;
    });
    renderTableData(data, ridesTable);
    document.getElementById("ridesLength").innerHTML = "".concat(data.length);
  }
});
